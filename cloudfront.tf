...
resource "aws_cloudfront_response_headers_policy" "security_headers_policy" {
  ...
  security_headers_config {
    ...
    content_security_policy {
      content_security_policy = "default-src 'self' data: https://*.data.gouv.fr https://openmaptiles.github.io https://cdn.jsdelivr.net https://anct-carto-client-feature-les-assembleurs.s3.eu-west-3.amazonaws.com; font-src 'self' https://cdn.jsdelivr.net; img-src 'self' data: https://*.data.gouv.fr; object-src 'none'; script-src 'self' 'unsafe-inline' 'unsafe-eval' https://*.data.gouv.fr blob:; style-src 'self' 'unsafe-inline';"
      override                = true
    }
  }
}
